...
void *readerEgoista(void *arg){
    ...
    while (true)
    {
        ...
        if (i == vectores)
        {
            printf("\e[92;1m: El reader egoista %ld no encuentra espacio \n", msj->pid);
        }
        else
        {
            // Obtener y mostrar la fecha
            ...
            update_bitacora( msj, action);
            printf("\e[92;1m Borrando egoistamente en la linea %d \n", msj->linea);
...
int main(int argc, char *argv[]) {
    ...
    printf("Selfish reader looking for space \n");
...
int main(int argc, char *argv[]) {    
    ...
    while ( i < num_readers)
    {
        ...
        if(pthread_create(&reader_thread, NULL, &readerEgoista, (void*)sett) != 0){
